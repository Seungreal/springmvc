				...
				<ul class="sidebar-nav">
					...
					<li class="sidebar-item">
						<a id="students" class="sidebar-link" href="#">
							<i class="align-middle" data-feather="user"></i> <span class="align-middle">학생목록</span>
						</a>
					</li>

					<li class="sidebar-item">
						<a id="teachers"class="sidebar-link" href="#">
							<i class="align-middle" data-feather="settings"></i> <span class="align-middle">교사목록</span>
						</a>
					</li>
		...
		$('#students').click(e=>{
			$.getJSON('/students',d=>{
				...
				if(d.length!=0){
					...
					tr = '<tr><td colspan="6"> 조회내용이 없습니다 </td></tr>'
				}
				$('main').html(`<h2>학생 목록</h2>
							<table>
							<thead>
							<tr>
								<th>학번</th>
			...
			$.getJSON('/teachers',d=>{
				...
				if(d.length!=0){
					...
					for(;i<d.length;i++){
						tr+=`<tr>
							<td>${d[i].teaNum}</td>
							<td><a href="#"class="names" id="${d[i].name}">${d[i].name}</a></td>
							<td>${d[i].subject}</td>
							<td>${d[i].email}</td>
							</tr>`
					}
				}else{}
				$('main').html(`<h2>교사 목록</h2>
							<table>
							<thead>
							<tr>
								<th>교번</th>
				...
				$('.names').each(function(){
					$(this).click(e=>{
						localStorage.setItem('searchName',this.id)
						location.href='/transfer/sym/mgr/teacher'
					})
				})
			})
		})
